...
# Open the file and read the content into lines
...
while t < len(chat_lines) - 1:
    chat_lines[t] = chat_lines[t].strip()  # Remove extra whitespace
    chat_lines[t] = chat_lines[t].replace("Ã¼", "ue").replace("â€Ž", "")  # Clean encoding artifacts
    
    # If the next line lacks a timestamp or  contains "omitted," merge it with the current line
    if not re.match(timestamp_pattern, chat_lines[t + 1]) and not "omitted" in chat_lines[t + 1]:
        chat_lines[t] += " " + chat_lines[t + 1]
        chat_lines.pop(t + 1)  # Remove the next line after merging
    else:        
        t += 1
...
for line in chat_lines:
    ...
    if colon_position != -1:
        ...
        if name not in participant_names and name not in excluded_names:
...
# Create Person instances for each participant
participant_objects = []
for participant_name in participant_names:
    #print(f"What gender is {participant_name}?(male, female, other)")
    gender = "other"  # Placeholder for user input
    participant_objects.append(Person(participant_name, gender))


# Parse each line to collect message timestamps for each participant
    ...
    for line in chat_lines:
        ...
        if line.startswith("["):
            ...
            if line[21:29] == participant:
                ...
                hour = int(line[11:13])
                participant_objects[person_index].hours.append(hour)
                
                minute = int(line[14:16])
                participant_objects[person_index].minutes.append(minute)

                second = int(line[17:19])
                participant_objects[person_index].seconds.append(second)

                day = int(line[1:3])
                participant_objects[person_index].days.append(day)

                month = int(line[4:6])
                participant_objects[person_index].months.append(month)

                year = int(line[7:9])
                participant_objects[person_index].years.append(year)
...
for person_index, participant in enumerate(participant_names):
    for line in chat_lines:
        ...
        if line.find(participant) >= 0:
            participant_objects[person_index].message_count += 1
            message_count_total += 1
            if line.count("image omitted")==1:
                participant_objects[person_index].image_count+=1 
                
            elif line.count("document omitted")==1:  
                participant_objects[person_index].file_count+=1 
                
            elif line.count("audio omitted")==1:  
                participant_objects[person_index].audio_count+=1 
                
            elif line.count("video omitted")==1:  
                participant_objects[person_index].video_count+=1 
                
            elif line.count("sticker omitted")==1:  
                participant_objects[person_index].sticker_count+=1
                
            elif line.count("This message was deleted")==1:
                participant_objects[person_index].deleted_count+=1
                
            elif line.count("ideo call")==1:
                participant_objects[person_index].video_call_count+=1
                
            elif line.count("oice call")==1:
                participant_objects[person_index].voice_call_count+=1
                
            else:
                message_start = line.find(participant) + len(participant)
                participant_objects[person_index].messages.append(line[message_start:])
                participant_objects[person_index].text_message_count += 1
                

            if line.count ("<This message was edited>")==1:
               participant_objects[person_index].edit_count+=1 
               
            if line.count ("http")>=1:
               participant_objects[person_index].link_count+=1 
               


# Creates a String with all the messages 
new_total_text_content= ""
for person in participant_objects:
    new_total_text_content+= " ".join(person.messages)

total_files = sum(person.file_count for person in participant_objects)
total_stickers = sum(person.sticker_count for person in participant_objects)
total_audios = sum(person.audio_count for person in participant_objects)
total_videos = sum(person.video_count for person in participant_objects)
total_images = sum(person.image_count for person in participant_objects)
total_edits = sum(person.edit_count for person in participant_objects)
total_deleted = sum(person.deleted_count for person in participant_objects)
total_video_calls = sum(person.video_call_count for person in participant_objects)
total_voice_calls = sum(person.voice_call_count for person in participant_objects)
total_links = sum(person.link_count for person in participant_objects)
total_words = len(new_total_text_content.split())

for person in participant_objects:
    person.calculate_message_percentage(message_count_total)
    person.calculate_word_percentage(total_words)
    person.calculate_file_percentage(total_files)
    person.calculate_sticker_percentage(total_stickers)
    person.calculate_audio_percentage(total_audios)
    person.calculate_video_percentage(total_videos)
    person.calculate_image_percentage(total_images)
    person.calculate_edit_percentage(total_edits)
    person.calculate_deleted_percentage(total_deleted)
    person.calculate_video_call_percentage(total_video_calls)
    person.calculate_voice_call_percentage(total_voice_calls)
    person.calculate_links_percentage(total_links)


# Final calculations for message and word percentages
for person_index, participant in enumerate(participant_names):
    person = participant_objects[person_index]
    ...
    person.message_percentage = person.message_count / message_count_total
    person.word_percentage = person.word_count / len(new_total_text_content.split()) #
